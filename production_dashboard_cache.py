    ...
    for o in all_orders:
        try:
            ...
            try:
                ...
                import traceback
                print(f"[打单缓存] 订单[{so_id}] infer_order_type 失败: {traceback.format_exc()}")
                order_type = "飞机盒"
            try:
            ...
            for line_idx, item in enumerate(o.get("items") or []):
                ...
                try:
                    ...
                    import traceback
                    print(f"[打单缓存] 子单[{so_id}][{line_idx}] 解析跳过: {traceback.format_exc()}")
                    continue
            ...
            import traceback
            sid = (so_id if isinstance(so_id, str) else str(so_id or "")) if "so_id" in dir() else str(o.get("so_id", ""))
            print(f"[打单缓存] 订单[{sid}] 整体处理跳过: {traceback.format_exc()}")
            failed_orders.append(sid)
            continue

    if failed_orders:
        print(f"[打单缓存] {len(failed_orders)} 个订单解析失败，已跳过: {failed_orders}")
