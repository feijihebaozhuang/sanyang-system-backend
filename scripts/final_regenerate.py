...
print(f'结果: 定制={len(custom_list)} 扣底盒/双插盒={len(kd_list)} 纸箱={len(zx_list)} 其他={len(other_list)}')
print(f'合计: {len(custom_list)+len(kd_list)+len(zx_list)+len(other_list)} = {total}')
...
for row in other_list:
    s = str(row.get('平台规格名称', '')).strip()
    dim = classify_dim(s)
    if dim and dim[0] == '小数':  # 含小数非全.5
        feiji_rows.append(row)
    else:
        remaining_other.append(row)

print(f'  非全量飞机盒(带小数点): {len(feiji_rows)} 条')
print(f'  其余商品剩余: {len(remaining_other)} 条')

save_xlsx(r'D:\Desktop\非全量飞机盒.xlsx', feiji_rows, '其余商品中带小数点的（已排除全.5）- 非全量飞机盒')
save_xlsx(r'D:\Desktop\其余商品.xlsx', remaining_other, '其余商品（已移除小数项）')

print('\n=== 完成 ===')
print(f'定制链接商品.xlsx: {len(custom_list)}')
print(f'扣底盒双插盒商品.xlsx: {len(kd_list)}')
print(f'纸箱商品.xlsx: {len(zx_list)}')
print(f'非全量飞机盒.xlsx: {len(feiji_rows)}')
print(f'其余商品.xlsx: {len(remaining_other)}')
print(f'合计: {len(custom_list)+len(kd_list)+len(zx_list)+len(feiji_rows)+len(remaining_other)}')
