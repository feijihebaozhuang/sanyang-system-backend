...
def parse_zh(s):
    ...
    # 2. 飞机盒【长度Lcm】【材料】;层;【宽度Wcm】X【高度Hcm】
    ...
    if m:
        w, h, l = float(m.group(1)), float(m.group(2)), int(m.group(4))
        mat = '白色'
        if '双面红' in s: mat = '特硬'
        elif '双面黑' in s: mat = '特硬'
        return (mat, sorted([l, w, h], reverse=True), '外径')
    
    # 4. 【E坑高度Hcm】材料;层;扣底盒【长X宽LxW】n个一组
    ...
    if m:
        return ('优质', sorted([round(float(m.group(1))), round(float(m.group(2))), round(float(m.group(3)))], reverse=True), '内径')
    ...
    if m: return ('特硬', sorted([int(m.group(4)), int(m.group(1)), int(m.group(2))], reverse=True), '内径')
