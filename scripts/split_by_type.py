# -*- coding: utf-8 -*-
"""
从平台商品中（排除定制链接1696条后），按品类拆分：
1. 扣底盒/双插盒 → 一个文件
2. 纸箱 → 一个文件
3. 剩下的 → 一个文件

每个文件再细分长宽高类型（含小数/全.5/整数）
"""
import sys, os
...
def is_custom(s):
    """按规格判断是否为定制链接（无准确尺寸）"""
    # 明确有尺寸的：有LxWxH格式 或 长宽高关键词+数字
    has_lwh_keyword = bool(re.search(r'长[度]*\s*[：:]?\s*【\s*[\d.]+', s)) or \
                      bool(re.search(r'[长宽高]\s*[：:]?\s*【\s*[\d.]+', s))
    ...
    # 只有"特硬;双插盒"这种简短描述没有具体尺寸的，也算定制
    nums = re.findall(r'[\d.]+', s)
    # 如果没有数字，说明是纯描述性链接
    if len(nums) == 0:
        return True
    
    # 有数字但根本不是长宽高格式（比如只有数量100个）
    if not has_dim_format and not has_cm_dim:
        # 检查数字是否可能是长宽高的一部分
        return True
