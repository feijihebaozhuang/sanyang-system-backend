...
for idx in range(len(df)):
    ...
    if l is None:
        nodim.append((shop, spec, pid, '', 0, 0, 0))
        continue
    ...
    if is_int and not is_nei:
        waijing.append((shop, spec, pid, '外径', l, w, h))
    elif (is_int and is_nei) or (is_all5 and not is_nei):
        neijing.append((shop, spec, pid, '内径' if is_nei else '外径', l, w, h))
    else:
        feiquan.append((shop, spec, pid, '内径' if is_nei else '外径', l, w, h))

print(f'匹配: {matched}, 未匹配: {not_matched}')
for name, lst in [('5a外径', waijing), ('5b内径', neijing), ('5c非全量', feiquan), ('5d未提取', nodim)]:
...
outdir = r'D:\Desktop'
write_file(waijing, f'{outdir}\\5a-外径飞机盒.txt', '外径飞机盒（外径+整数）')
write_file(neijing, f'{outdir}\\5b-内径飞机盒.txt', '内径飞机盒（内径+整数 或 外径+.5）')
write_file(feiquan, f'{outdir}\\5c-非全量飞机盒.txt', '非全量飞机盒（其他小数）')
write_file(nodim, f'{outdir}\\5d-未提取尺寸.txt', '未提取到尺寸')

print('\n✅ 全部完成！')
print('1/2/3/4/4a/5/6 未被修改。')
