...
def normalize_dim(v_str):
    v_str = v_str.strip(); unit = ''; val_str = v_str
...
def is_custom(s):
    ...
    # 只有长*宽【*】没有高的（如珍珠棉厚度类），也是定制
    # 检查是否有完整三维尺寸 LxWxH
    has_3d = bool(re.search(r'[\d.]+\s*[xX*×]\s*[\d.]+\s*[xX*×]\s*[\d.]+', s))
    has_3d_in_bracket = bool(re.search(r'【\s*[\d.]+\s*[xX*×]\s*[\d.]+\s*[xX*×]\s*[\d.]+\s*】', s))
    has_lwh_keyword = bool(re.search(r'[长宽高][度]*\s*[：:]?\s*【\s*[\d.]+', s))
    
    # 有完整三维尺寸→不是定制
    if has_3d: return False
    
    # 没有数字的→定制
    nums = re.findall(r'[\d.]+', s)
    if len(nums) == 0: return True
    
    # 有【】但里面只有2个数（长*宽，缺高）→定制
    # 只有1个数的也→定制
    if not has_3d_in_bracket and not has_lwh_keyword:
        return True
