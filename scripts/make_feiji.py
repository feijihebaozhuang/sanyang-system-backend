# -*- coding: utf-8 -*-
"""
从其余商品.xlsx中分出带小数点的（排除全.5）→ 非全量飞机盒.xlsx
"""
import sys, os
...
print(f'其余商品总: {len(feiji) + len(remaining)}')
...
wb2.save(feiji_file)
print(f'✅ 非全量飞机盒.xlsx: {len(feiji)}条')
...
wb3.save(other_file)
print(f'✅ 其余商品已更新: {len(remaining)}条')

print(f'\n验证总数: {256+7266+33214+len(feiji)+len(remaining)}')
