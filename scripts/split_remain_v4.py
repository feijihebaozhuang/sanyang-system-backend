...
with open(r'D:\Desktop\5-剩余商品结构.txt', 'r', encoding='utf-8') as f:
    for line in f:
        ...
        if m:
            REMAIN_SKELETONS.add(m.group(1).strip())
...
for idx in range(len(df)):
    ...
    is_quanliang = is_int or is_all5
    ...
    if is_int and not is_nei:
        ...
        # 内径+整数、外径+.5 → 5b内径飞机盒
        buckets['内径飞机盒'].append((shop, spec, pid, '内径' if is_nei else '外径', l, w, h))
    else:
        # 其他全量（内径+.5）或非全量（小数）→ 5c非全量飞机盒
        buckets['非全量飞机盒'].append((shop, spec, pid, '内径' if is_nei else '外径', l, w, h))

print(f'\n排除: {excluded}, 匹配: {matched}, 未匹配: {not_matched}, 合计: {excluded+matched+not_matched} (应={total})')
...
outdir = r'D:\Desktop'
write_file(buckets['外径飞机盒'], f'{outdir}\\5a-外径飞机盒.txt', '外径飞机盒（外径+全量：整数或.5）')
write_file(buckets['内径飞机盒'], f'{outdir}\\5b-内径飞机盒.txt', '内径飞机盒（内径+全量：整数或.5）')
