...
def _fd(s):
    """提取干净的数字列表：跳过数量标记和字母前缀"""
    s = re.sub(r'【\d+[个起拍][^】【]*】', '【】', s)  # 移除【N个】【N起拍】（有闭合】
    s = re.sub(r'【\d+[个起拍].*?(?=【|$)', '', s)  # 移除【N个起拍 直到下一个【或结尾
    s = re.sub(r'\d+[个起拍]', '', s)                 # 移除裸数量标记如100个（无【】）
    s = re.sub(r'([TBA])(\d+)\(', r'\1(', s)         # 移除T1( → T( 等
...
def p9(s):
    ...
    w = grab_num(s, after='宽度【', before='mm')
    l = grab_num(s, after='长度【', before='mm')
    h = grab_num(s, after=';【', before='mm')
    if not l or not w or not h: return None
...
# ===== 第5组: mm尺寸 台湾纸/双面白 系列 =====
def p11_14(s):
    """mm系列：3数值/10转cm。跳过数量和尺寸顺序修复"""
    # 先检查是否有"宽度"（宽度在前的尺寸顺序不同）
    if '宽度【' in s:
        w = grab_num(s, after='宽度【', before='mm')
        l = grab_num(s, after='长度【', before='mm')
        h = grab_num(s, after='高度【', before='mm')
        if not l or not w or not h: return None
        return (l/10, w/10, h/10)  # 返回(长度, 宽度, 高度)
...
def p37(s):
    ...
    nums = [float(x) for x in _fd(s)]
    if len(nums) < 3: return None
    return (nums[1]/10, nums[0]/10, nums[2]/10)
...
# ===== p65 =====
def p65(s):
    """宽度【Nmm】N个;长度【Nmm】双面白;高度【Nmm外尺寸】"""
    w = grab_num(s, after='宽度【', before='mm')
    l = grab_num(s, after='长度【', before='mm')
    h = grab_num(s, after='高度【', before='mm')
    if not l or not w or not h:
...
def p66_68(s):
    nums = [float(x) for x in _fd(s)]
    if len(nums) < 3: return None
    # 白色;NxNcm外径【N个】;Ncm → [宽, 高, 长]  _fd已跳数量
...
def p115_155_156(s):
    ...
    if len(nums) < 3: return None
    # 特硬【日本牛卡】外径;N-N-Nmm【N个】 → 都是尺寸，3个数
